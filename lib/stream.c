...
struct stream *
stream_copy (struct stream *new1, struct stream *src)
{
  STREAM_VERIFY_SANE (src);
  
  assert (new1 != NULL);
  assert (STREAM_SIZE(new1) >= src->endp);

  new1->endp = src->endp;
  new1->getp = src->getp;
  
  memcpy (new1->data, src->data, src->endp);
...
stream_dup (struct stream *s)
{
  ...
  if ( (new1 = stream_new (s->endp)) == NULL)
    return NULL;

  return (stream_copy (new1, s));
}
...
stream_dupcat (struct stream *s1, struct stream *s2, size_t offset)
{
  ...
  if ( (new1 = stream_new (s1->endp + s2->endp)) == NULL)
    return NULL;

  memcpy (new1->data, s1->data, offset);
  memcpy (new1->data + offset, s2->data, s2->endp);
  memcpy (new1->data + offset + s2->endp, s1->data + offset,
	  (s1->endp - offset));
  new1->endp = s1->endp + s2->endp;
  return new1;
}
...
// added by chahn
int
stream_put_prefix_name (struct stream *s, struct prefix *p)
{
  size_t psize;
  ...
  s->data[s->endp++] = psize;
  memcpy (s->data + s->endp, p->name, psize);
  s->endp += psize;
...
{
  struct stream_fifo *new1;
 
  new1 = (struct stream_fifo *)XCALLOC (MTYPE_STREAM_FIFO, sizeof (struct stream_fifo));
  return new1;
}
