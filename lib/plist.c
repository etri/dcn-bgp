...
{
  struct prefix_list *new1;

  new1 = (struct prefix_list *)XCALLOC (MTYPE_PREFIX_LIST, sizeof (struct prefix_list));
  return new1;
}
...
{
  struct prefix_list_entry *new1;

  new1 = XCALLOC (MTYPE_PREFIX_LIST_ENTRY, sizeof (struct prefix_list_entry));
  return new1;
}
			...
			struct prefix_list_entry *new1)
{
  ...
  for (pentry = plist->head; pentry; pentry = pentry->next)
    {
      if (prefix_same (&pentry->prefix, &new1->prefix)
	  && pentry->type == new1->type
	  && pentry->le == new1->le
	  && pentry->ge == new1->ge
	  && pentry->seq != seq)
	return pentry;
