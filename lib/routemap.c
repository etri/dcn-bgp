...
{
  struct route_map *new1;

  new1 =  XCALLOC (MTYPE_ROUTE_MAP, sizeof (struct route_map));
  new1->name = XSTRDUP (MTYPE_ROUTE_MAP_NAME, name);
  return new1;
}
...
{
  struct route_map_index *new1;

  new1 =  XCALLOC (MTYPE_ROUTE_MAP_INDEX, sizeof (struct route_map_index));
  new1->exitpolicy = RMAP_EXIT; /* Default to Cisco-style */
  return new1;
}
...
{
  struct route_map_rule *new1;

  new1 = XCALLOC (MTYPE_ROUTE_MAP_RULE, sizeof (struct route_map_rule));
  return new1;
}
