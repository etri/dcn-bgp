...
{
  struct work_queue *new1;
  
  new1 = XCALLOC (MTYPE_WORK_QUEUE, sizeof (struct work_queue));

  if (new1 == NULL)
    return new1;
  
  new1->name = XSTRDUP (MTYPE_WORK_QUEUE_NAME, queue_name);
  new1->master = m;
  SET_FLAG (new1->flags, WQ_UNPLUGGED);
  
  if ( (new1->items = list_new ()) == NULL)
    {
      XFREE (MTYPE_WORK_QUEUE_NAME, new1->name);
      XFREE (MTYPE_WORK_QUEUE, new1);
      
      return NULL;
    }
  
  new1->items->del = (void (*)(void *)) work_queue_item_free;  
  
  listnode_add (work_queues, new1);
  
  new1->cycles.granularity = WORK_QUEUE_MIN_GRANULARITY;
  new1->cycles.worst = UINT_MAX;
  
  /* Default values, can be overriden by caller */
  new1->spec.hold = WORK_QUEUE_DEFAULT_HOLD;
