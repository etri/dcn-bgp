...
{
  unsigned int size;
  vector new1 = XCALLOC (MTYPE_VECTOR, sizeof (struct _vector));

  new1->active = v->active;
  new1->alloced = v->alloced;

  size = sizeof (void *) * (v->alloced);
  new1->index = XCALLOC (MTYPE_VECTOR_INDEX, size);
  memcpy (new1->index, v->index, size);
