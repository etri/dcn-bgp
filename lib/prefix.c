...
/* When string format is invalid return 0. */
int
str2prefix_ndn (const char *str, struct prefix_ndn *p)
{
	p->family = AF_NDN;
	p->prefixlen = strlen(str);
...
str2prefix (const char *str, struct prefix *p)
{
  ...
  // added by chahn
  /* Next we try to convert string to struct prefix_ndn. */
  ret = str2prefix_ndn (str, (struct prefix_ndn *) p);
  strcpy(p->name, str);
  if (ret)
    return ret;
