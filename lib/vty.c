...
vty_new ()
{
  struct vty *new1 = XCALLOC (MTYPE_VTY, sizeof (struct vty));

  new1->obuf = buffer_new(0);	/* Use default buffer size. */
  new1->buf = XCALLOC (MTYPE_VTY, VTY_BUFSIZ);
  new1->max = VTY_BUFSIZ;
...
vty_flush (struct thread *thread)
{
  ...
  erase = ((vty->status == vty::VTY_MORE || vty->status == vty::VTY_MORELINE));
  ...
  else if (vty->status == vty::VTY_MORELINE)
...
{
  return vty->type == vty::VTY_SHELL ? 1 : 0;
}

int
vty_shell_serv (struct vty *vty)
{
  return vty->type == vty::VTY_SHELL_SERV ? 1 : 0;
}
