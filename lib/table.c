...
static void
route_common (const struct prefix *n, const struct prefix *p, struct prefix *new1)
{
  int i;
  u_char diff;
  u_char mask;
  ...
  u_char *newp = (u_char *)&new1->u.prefix;
  ...
  if (new1->prefixlen != p->prefixlen)
    {
      diff = np[i] ^ pp[i];
      mask = 0x80;
      while (new1->prefixlen < p->prefixlen && !(mask & diff))
	{
	  mask >>= 1;
	  new1->prefixlen++;
	}
      newp[i] = np[i] & maskbit[new1->prefixlen % 8];
    }
}

static void
set_link (struct route_node *node, struct route_node *new1)
{
  unsigned int bit = prefix_bit (&new1->p.u.prefix, node->p.prefixlen);
...
#else
      if (strcmp(node->p.name, p->name) == 0)
#endif
        break;
...
route_node_get (struct route_table *const table, const struct prefix *p)
{
  ...
  if (node == NULL)
    ...
    {
      new1 = route_node_new (table);
      route_common (&node->p, p, &new1->p);
      new1->p.family = p->family;
      new1->table = table;
      set_link (new1, node);
	...
	table->top = new1;
      ...
      if (strcmp(new1->p.name, p->name) != 0)
	{
	  match = new1;
	  new1 = route_node_set (table, p);
	  set_link (match, new1);
