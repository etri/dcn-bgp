...
static struct stream *
dcn_bgp_update_packet (struct peer *peer, afi_t afi, safi_t safi)
{
	struct stream *s;
	struct stream *snlri;
	struct bgp_adj_out *adj;
	...
	while (adv)
	{
		...
		if (stream_empty (s))
		{
			...
			/* 5: Encode all the attributes, except MP_REACH_NLRI attr. */
			total_attr_len = dcn_bgp_packet_attribute (NULL, peer, s,
					adv->baa->attr,
					((afi == AFI_IP && safi == SAFI_UNICAST) ?
					 &rn->p : NULL),
					afi, safi,
...
static struct stream *
dcn_bgp_withdraw_packet (struct peer *peer, afi_t afi, safi_t safi)
{
  struct stream *s;
  struct stream *packet;
  struct bgp_adj_out *adj;
...
static struct stream *
dcn_bgp_write_packet (struct peer *peer)
{
	int afi;
	safi_t safi;
	struct bgp_advertise *adv;
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi++) 
	{
		for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
		{
			...
			if (adv)
			{
				s = dcn_bgp_withdraw_packet (peer, afi, safi);
				if (s)
					return s;
			}
		}
			...
			if (adv)
			{
				if (adv->binfo && adv->binfo->uptime < peer->synctime)
				{
							...
							&& safi != SAFI_MPLS_VPN)
					{
						if (CHECK_FLAG (adv->binfo->peer->af_sflags[afi][safi],
									PEER_STATUS_EOR_RECEIVED)) {
							s = dcn_bgp_update_packet (peer, afi, safi);
						}
					}
					else {
						s = dcn_bgp_update_packet (peer, afi, safi);
...
bgp_write (struct thread *thread)
{
  ...
  std::cout << "bgp_write: " << peer->remote_name << std::endl;
...
int
dcn_bgp_write(struct peer *peer)
{
	struct stream *s = NULL;
	int length = 0;

	s = dcn_bgp_write_packet(peer);
	if (!s)
		return 0;

	if (s) {
		length = bgp_packet_set_size(s);
		get_dcn_master()->send_bgp_message(peer, length, (uint8_t *)s->data);
	}
...
bgp_keepalive_send (struct peer *peer)
{
  ...
  if (BGP_DEBUG (keepalive, KEEPALIVE))  
    zlog_debug ("%s sending KEEPALIVE", peer->remote_name); 
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
               peer->remote_name, BGP_MSG_KEEPALIVE, length);
  ...
  //added by MODORI on 20210927
  //BGP_WRITE_ON (peer->t_write, bgp_write, peer->face_id);
...
bgp_open_send (struct peer *peer)
{
  ...
  stream_putw (s, strlen(peer->local_name)+1); /* BGP ID Length */
  stream_put(s, peer->local_name, strlen(peer->local_name)+1); /* BGP Identifier */
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s sending OPEN, version %d, my as %u, holdtime %d, id %s", 
	       peer->remote_name, BGP_VERSION_4, local_as,
	       send_holdtime, peer->local_name);

  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
	       peer->remote_name, BGP_MSG_OPEN, length);
			   ...
			   u_char *data, size_t datalen)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
	       peer->remote_name, BGP_MSG_NOTIFY, length);
  ...
  if (sub_code != BGP_NOTIFY_CEASE_CONFIG_CHANGE)
    {
      if (sub_code == BGP_NOTIFY_CEASE_ADMIN_RESET)
      {
        ...
        zlog_info ("Notification sent to neighbor %s:%u: User reset",
                   peer->remote_name, sockunion_get_port (&peer->su));
      }
		     ...
		     int capability_code, int action)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
	       peer->remote_name, BGP_MSG_CAPABILITY, length);
...
static int
bgp_collision_detect (struct peer *new1, struct in_addr remote_id)
{
  struct peer *peer;
  ...
  for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
      if (peer == new1)
        continue;
	  if (strcmp(peer->remote_name, new1->remote_name) != 0)
        continue;
#if 0
      if (!sockunion_same (&peer->su, &new1->su))
        continue;
#endif
          ...
          if (new1->fd >= 0)
            {
              if (BGP_DEBUG (events, EVENTS))
                 zlog_debug ("%s:%u Existing Established peer, sending NOTIFY",
                             new1->host, sockunion_get_port (&new1->su));
              bgp_notify_send (new1, BGP_NOTIFY_CEASE, 
      ...
      if (peer->status == OpenConfirm || peer->status == OpenSent)
	{
	  ...
	  if (!CHECK_FLAG (new1->sflags, PEER_STATUS_ACCEPT_PEER))
	    {
	      out = new1;
	      ret_close_out = -1;
	      in = peer;
...
bgp_open_receive (struct peer *peer, bgp_size_t size)
{
  ...
  char remote_name[BGP_ROUTER_NAME_SIZE];
  ...
  id_length = stream_getw (peer->ibuf);
  stream_get(remote_name, peer->ibuf, id_length);

  /* Receive OPEN message log  */
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s rcv OPEN, version %d, remote-as (in open) %u,"
                " holdtime %d, id %s, %sbound connection",
	        peer->remote_name, version, remote_as, holdtime,
	        remote_name,
  ...
  if (remote_as == BGP_AS_TRANS)
    {
      ...
      if (!as4 && BGP_DEBUG (as4, AS4))
        zlog_debug ("%s [AS4] OPEN remote_as is AS_TRANS, but no AS4."
                    " Odd, but proceeding.", peer->remote_name);
      else if (as4 < BGP_AS_MAX && BGP_DEBUG (as4, AS4))
        zlog_debug ("%s [AS4] OPEN remote_as is AS_TRANS, but AS4 (%u) fits "
                    "in 2-bytes, very odd peer.", peer->remote_name, as4);
      if (as4)
        remote_as = as4;
    } 
  else 
    {
	  ...
	  zlog_err ("%s bad OPEN, got AS4 capability, but remote_as %u"
	            ...
	            peer->remote_name, as4, remote_as);
	  bgp_notify_send (peer, BGP_NOTIFY_OPEN_ERR,
			   BGP_NOTIFY_OPEN_BAD_PEER_AS);
	  return -1;
	}
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
      ...
      if (! realpeer)
	{
	  ...
	  if (as)
	    {
	      if (BGP_DEBUG (normal, NORMAL))
		zlog_debug ("%s bad OPEN, 1. wrong router identifier %s",
			    peer->remote_name, inet_ntoa (remote_id));
		...
		zlog_debug ("%s bad OPEN, remote AS is %u, expected %u",
			    peer->remote_name, remote_as, peer->as);
	      bgp_notify_send_with_data (peer, BGP_NOTIFY_OPEN_ERR,
					 BGP_NOTIFY_OPEN_BAD_PEER_AS,
  ...
  if (remote_id.s_addr == 0
    ...
    {
      if (BGP_DEBUG (normal, NORMAL))
	zlog_debug ("%s bad OPEN, 2. wrong router identifier %s",
		   peer->remote_name, inet_ntoa (remote_id));
  ...
  if (version != BGP_VERSION_4)
	...
	zlog_debug ("%s bad protocol version, remote requested %d, local request %d",
		   peer->remote_name, version, BGP_VERSION_4);
  ...
  switch (packet->safi)
    {
      ...
      case SAFI_ENCAP:
        ...
        // Added by MODORI 20210908
      case SAFI_NDN:
        return bgp_nlri_parse_ndn (peer, attr, packet);
    }
  return -1;
}
...
// added by chahn
static int
dcn_bgp_update_receive (struct peer *peer, bgp_size_t size)
{
  int ret, nlri_ret;
  u_char *end;
  struct stream *s;
...
bgp_notify_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (peer->status == Established) {
	...
	if (bgp_notify.code == BGP_NOTIFY_CEASE &&
			bgp_notify.subcode == BGP_NOTIFY_CEASE_PEER_UNCONFIG) {
		if (CHECK_FLAG (peer->sflags, PEER_STATUS_NSF_MODE))
		{
...
int bgp_write_to_dcn(char * to, uint8_t * message)
{
    struct bgp_header *hdr = (struct bgp_header*)message;
    //get_dcn_master()->send_bgp_message(to, hdr->type, hdr->length, message);
    return 0;
}
int bgp_redistribute_from_dcn (const char *prefix_list, size_t length, bool isNo)
{
	zlog_debug ("BGP bgp_redistribute_from_dcn - L: %d, isNo:%d", length, isNo);
    char *prefix = strtok(prefix_list, ",");
    int cnt = 0;
    while(prefix!=NULL){
	zlog_debug ("[%d] - %s", ++cnt, prefix);
		struct prefix p;
		memset (&p, 0, sizeof (struct prefix_ndn));
		p.family = AF_NDN;
		p.prefixlen = strlen(prefix);
		strcpy(p.name, prefix);
		if (isNo)
			dcn_bgp_redistribute_delete((struct prefix *)&p, ZEBRA_ROUTE_STATIC);
		else
			dcn_bgp_redistribute_add((struct prefix *)&p, 0, ZEBRA_ROUTE_STATIC, 0);
        prefix = strtok(NULL, ",");
    }

}
//added by MODORI on 20210910
int bgp_read_from_dcn(struct peer * peer, uint8_t type, size_t length, const uint8_t * message)
{
	zlog (peer->log, LOG_DEBUG, "BGP bgp_read_from_dcn - T: %d, L: %d", type, length);
	...
	switch (bgp_type)
	{
		...
		case BGP_MSG_UPDATE:
			peer->readtime = bgp_recent_clock ();
			dcn_bgp_update_receive (peer, size);
			break;
