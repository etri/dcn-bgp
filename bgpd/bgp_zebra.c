...
// added by chahn
int
dcn_bgp_nexthop_set (struct bgp_nexthop *nexthop, struct peer *peer)
{
  memset (nexthop, 0, sizeof (struct bgp_nexthop));

  strcpy(nexthop->name, peer->local_name);
  nexthop->face_id = peer->face_id;
...
/* Other routes redistribution into BGP. */
 int
 dcn_bgp_redistribute_set (struct bgp *bgp, afi_t afi, int type)
 {
...
/* added by MODORI on 20211118 DcnUnset redistribution.  */
int
dcn_bgp_redistribute_unset (struct bgp *bgp, afi_t afi, int type)
{
    /* Unset flag from BGP instance. */
    bgp->redist[afi][type] = 0;
