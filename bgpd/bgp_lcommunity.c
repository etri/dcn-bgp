...
lcommunity_parse (u_int8_t *pnt, u_short length)
{
  ...
  /* Create a new Large Communities Attribute by uniq and sort each
     Large Communities value  */
  new1 = lcommunity_uniq_sort (&tmp);

  return lcommunity_intern (new1);
}
...
lcommunity_dup (struct lcommunity *lcom)
{
  ...
  new1 = XCALLOC (MTYPE_LCOMMUNITY, sizeof (struct lcommunity));
  new1->size = lcom->size;
  if (new1->size)
    {
      new1->val = XMALLOC (MTYPE_LCOMMUNITY_VAL, lcom->size * LCOMMUNITY_SIZE);
      memcpy (new1->val, lcom->val, lcom->size * LCOMMUNITY_SIZE);
    }
  else
    new1->val = NULL;
  return new1;
