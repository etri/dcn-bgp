...
static int
bgp_router_name_set (struct bgp *bgp, const char *name)
{
    struct peer *peer;
    struct listnode *node, *nnode;
    if(name)
    	zlog_debug("bgp_router_name_set: %s" , name );
#if 0
    if (bgp_config_check (bgp, BGP_CONFIG_ROUTER_ID)
            && IPV4_ADDR_SAME (&bgp->router_name, id))
        return 0;
    ...
    {
        IPV4_ADDR_COPY (&peer->local_name, id);
    ...
    if ( name!=NULL ){ 
            if(bgp_config_check (bgp, BGP_CONFIG_ROUTER_ID)
            && bgp->router_name!=NULL && !strcmp (bgp->router_name, name))
    ...
    //IPV4_ADDR_COPY (&bgp->router_id, id);
    if(name != NULL){
        bgp->router_name = strdup(name);
        bgp_config_set (bgp, BGP_CONFIG_ROUTER_ID);
    }else
        bgp->router_name = NULL;
...
int
bgp_router_name_static_set (struct bgp *bgp, const char * name)
{
    if( name == NULL )
        bgp->router_name_static = NULL;
    else
        bgp->router_name_static = strdup(name);

    bgp_router_name_set (bgp, name);
    return 0;
}

/* BGP's cluster-id control. */
int
dcn_bgp_cluster_id_set (struct bgp *bgp, const char *cluster_name)
{
  struct peer *peer;
  struct listnode *node, *nnode;
  ...
  if (bgp_config_check (bgp, BGP_CONFIG_CLUSTER_ID)){
      ...
      if( bgp->cluster_name && !strcmp(bgp->cluster_name, cluster_name) )
        return 0;
  }

  //IPV4_ADDR_COPY (&bgp->cluster_id, cluster_id);
  bgp->cluster_name = strdup(cluster_name);
...
peer_new (struct bgp *bgp)
{
  ...
  peer->remote_name = NULL;
  peer->local_name = NULL;

  /* Set default value. */
  peer->fd = -1;
  // added by MODORI on 20211103
  peer->face_id = 0;
...
//added by MODORI on 20210924
static struct peer *
dcn_peer_create (const char *router_name, struct bgp *bgp, as_t local_as,
	     //as_t remote_as, afi_t afi, safi_t safi, uint64_t face_id)
	     as_t remote_as, afi_t afi, safi_t safi, const char *uri)
{
  int active;
  struct peer *peer;
  char buf[256]={0,};
  ...
  //peer->su = *su;
	peer->su.sa.sa_family = AF_NDN;

    peer->remote_name = strdup(router_name);  
    if(bgp->router_name!=NULL)
	peer->local_name = strdup(bgp->router_name);
    else{
        return NULL;
    }
  peer->local_as = local_as;
  ...
  listnode_add_sort (bgp->peer, peer);
    ...
    uint64_t face_id= get_dcn_master()->register_peer_router(router_name, uri, peer);
	peer->face_id = face_id;
  ...
  /* Make peer's address string. */
  // added by MODORI on 20210927: must be checked
  //sockunion2str (su, buf, SU_ADDRSTRLEN);
  get_dcn_master()->get_face_uri(face_id, buf);
  peer->host = XSTRDUP (MTYPE_BGP_PEER_HOST, 
		  buf
		  );
...
// added by MODORI on 20210923
int
dcn_peer_remote_as (struct bgp *bgp, const char *router_name, as_t *as,
        //afi_t afi, safi_t safi, uint64_t face_id)
        afi_t afi, safi_t safi, const char *uri)
{
    ...
    else
    {
        ...
        if (bgp_flag_check (bgp, BGP_FLAG_NO_DEFAULT_IPV4)
                && afi == AFI_NDN && safi == SAFI_NDN)
            peer = dcn_peer_create (router_name, bgp, local_as, *as, 0, 0, uri); 
        else
            peer = dcn_peer_create (router_name, bgp, local_as, *as, afi, safi, uri); 

            if(peer==NULL)
	            zlog_debug ("You should set router-name first.");
    }
    return 0;
}
...
// added by MODORI on 20210921
struct peer *
dcn_peer_lookup (struct bgp *bgp, const char *router_name)
{
    struct peer *peer;
    struct listnode *node, *nnode;
    ...
    {
		for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
			if (strcmp(peer->remote_name, router_name) == 0
...
peer_active_nego (struct peer *peer)
{
  if (peer->afc_nego[AFI_IP][SAFI_UNICAST]
      ...
      || peer->afc_nego[AFI_NDN][SAFI_NDN]
...
bgp_config_write (struct vty *vty)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, mnode, mnnode, bgp))
	...
	vty_out (vty, " bgp cluster-name %s%s", bgp->cluster_name,
		 VTY_NEWLINE);
