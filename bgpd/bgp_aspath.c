...
{
  struct assegment *new1;
  
  new1 = XCALLOC (MTYPE_AS_SEG, sizeof (struct assegment));
  
  if (length)
    new1->as = assegment_data_new (length);
...
assegment_dup (struct assegment *seg)
{
  ...
  new1 = assegment_new (seg->type, seg->length);
  memcpy (new1->as, seg->as, ASSEGMENT_DATA_SIZE (new1->length, 1) );
  ...
  while (seg)
    {
      if (head)
        {
          new1->next = assegment_dup (seg);
          new1 = new1->next;
        }
      else
        head = new1 = assegment_dup (seg);
...
aspath_dup (struct aspath *aspath)
{
  ...
  new1 = XCALLOC (MTYPE_AS_PATH, sizeof (struct aspath));

  if (aspath->segments)
    new1->segments = assegment_dup_all (aspath->segments);

  if (!aspath->str)
    return new1;

  new1->str = XMALLOC (MTYPE_AS_STR, buflen);
  new1->str_len = aspath->str_len;

  /* copy the string data */
  if (aspath->str_len > 0)
    memcpy (new1->str, aspath->str, buflen);
  else
    new1->str[0] = '\0';
...
aspath_hash_alloc (void *arg)
{
  ...
  new1 = XMALLOC (MTYPE_AS_PATH, sizeof (struct aspath));

  /* Reuse segments and string representation */
  new1->refcnt = 0;
  new1->segments = aspath->segments;
  new1->str = aspath->str;
  new1->str_len = aspath->str_len;
...
{
  struct assegment *last, *new1;

  if (! as1 || ! as2)
    return NULL;

  last = new1 = assegment_dup_all (as1->segments);
...
{
  struct assegment *seg = as->segments;
  struct assegment *new1 = assegment_new (type, 0);
