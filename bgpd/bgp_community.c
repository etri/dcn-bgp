  ...
  for (i = 0; i < com->size; i++)
    {
      ...
      if (! community_include (new1, val))
	community_add_val (new1, val);
    }

  qsort (new1->val, new1->size, sizeof (u_int32_t), community_compare);
...
community_parse (u_int32_t *pnt, u_short length)
{
  ...
  new1 = community_uniq_sort (&tmp);

  return community_intern (new1);
}
...
community_dup (struct community *com)
{
  ...
  new1 = XCALLOC (MTYPE_COMMUNITY, sizeof (struct community));
  new1->size = com->size;
  if (new1->size)
    {
      new1->val = XMALLOC (MTYPE_COMMUNITY_VAL, com->size * 4);
      memcpy (new1->val, com->val, com->size * 4);
    }
  else
    new1->val = NULL;
  return new1;
