...
cluster_parse (uint8_t * pnt, int length)
{
...
int
cluster_loop_check (struct cluster_list *cluster, char *originator)
{
  int i = 0;
 
  while (i < cluster->length) {
	  uint8_t name_length = cluster->list[i];
	  i++;
	  char name[PREFIX_NAME_LEN];
	  memcpy(name, &cluster->list[i], name_length);
	  if (strcmp(name, originator) == 0)
		  return 1;
  }
  return 0;
}
 ...
 */
void
bgp_attr_dup (struct attr *new1, struct attr *orig)
{
  struct attr_extra *extra = new1->extra;
  ...
  *new1 = *orig;
  ...
  if (extra)
    {
      new1->extra = extra;
      memset(new1->extra, 0, sizeof(struct attr_extra));
      if (orig->extra) {
        *new1->extra = *orig->extra;
        if (orig->extra->encap_subtlvs) {
          new1->extra->encap_subtlvs = encap_tlv_dup(orig->extra->encap_subtlvs);
        }
      }
    }
  else if (orig->extra)
    {
      new1->extra = bgp_attr_extra_new();
      *new1->extra = *orig->extra;
      if (orig->extra->encap_subtlvs) {
	new1->extra->encap_subtlvs = encap_tlv_dup(orig->extra->encap_subtlvs);
      }
...
#define MIX(val)	key = jhash_1word(val, key)
  ...
  MIX(attr->med);
  MIX(attr->local_pref);
  key = jhash(attr->name, strlen(attr->name), key);
  ...
  key += attr->local_pref;
  // added by chahn
  key += jhash(attr->name, strlen(attr->name), 0);
  ...
  if (extra)
    {
      ...
      MIX(extra->tag);
      key = jhash(extra->originator_name, strlen(extra->originator_name), key);
    }
...
attrhash_cmp (const void *p1, const void *p2)
{
  ...
  if (attr1->flag == attr2->flag
      ...
      && strcmp(attr1->name, attr2->name) == 0
...
bgp_attr_default_intern (u_char origin)
{
  ...
  new1 = bgp_attr_intern (&attr);
  bgp_attr_extra_free (&attr);
  
  aspath_unintern (&new1->aspath);
  return new1;
}
 ...
 */
const u_int8_t attr_flags_values [BGP_ATTR_LARGE_COMMUNITIES+1] = {
    // MODORI done
  ...
  [BGP_ATTR_DPA] =              BGP_ATTR_FLAG_OPTIONAL,
  [BGP_ATTR_ADVERTISER] =       BGP_ATTR_FLAG_OPTIONAL,
  [BGP_ATTR_RCID_PATH] =        BGP_ATTR_FLAG_OPTIONAL,
  ...
  [19] =   BGP_ATTR_FLAG_OPTIONAL ,
  [20] =   BGP_ATTR_FLAG_OPTIONAL ,
  [BGP_ATTR_AS_PATHLIMIT]= BGP_ATTR_FLAG_TRANS | BGP_ATTR_FLAG_OPTIONAL,
  [22]= BGP_ATTR_FLAG_OPTIONAL, 
  [BGP_ATTR_ENCAP] =            BGP_ATTR_FLAG_OPTIONAL,
  [24] =            BGP_ATTR_FLAG_OPTIONAL,
  [25] =            BGP_ATTR_FLAG_OPTIONAL,
  [26] =            BGP_ATTR_FLAG_OPTIONAL,
  [27] =            BGP_ATTR_FLAG_OPTIONAL,
  [28] =            BGP_ATTR_FLAG_OPTIONAL,
  [29] =            BGP_ATTR_FLAG_OPTIONAL,
  [30] =            BGP_ATTR_FLAG_OPTIONAL,
  [31] =            BGP_ATTR_FLAG_OPTIONAL,
  [BGP_ATTR_LARGE_COMMUNITIES]= BGP_ATTR_FLAG_OPTIONAL,
};
...
bgp_attr_originator_id (struct bgp_attr_parser_args *args)
{
  ...
  stream_get((bgp_attr_extra_get (attr))->originator_name, peer->ibuf, length);
...
bgp_attr_cluster_list (struct bgp_attr_parser_args *args)
{
  ...
  (bgp_attr_extra_get (attr))->cluster 
    = cluster_parse ((uint8_t *)stream_pnt (peer->ibuf), length);
...
#define LEN_LEFT	(length - (stream_get_getp(s) - start))
  ...
  stream_get(attr->name, s, attre->mp_nexthop_len);
#if 0
  ...
  switch (afi)
	...
	case AFI_NDN:
	  switch (safi)
	  {
		  case SAFI_NDN:
			  {
				  stream_putc(s, strlen(attr->name)+1);
				  stream_put(s, attr->name, strlen(attr->name)+1);
			  }
...
/* Make attribute packet. */
bgp_size_t
dcn_bgp_packet_attribute (struct bgp *bgp, struct peer *peer,
		      struct stream *s, struct attr *attr,
		      struct prefix *p, afi_t afi, safi_t safi,
		      struct peer *from, struct prefix_rd *prd, u_char *tag)
{
  ...
  if (peer->sort == BGP_PEER_IBGP
      ...
      && from->sort == BGP_PEER_IBGP)
    {
      ...
      if (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)) {
		  stream_putc (s, strlen(attr->extra->originator_name));
		  stream_put (s, attr->extra->originator_name, strlen(attr->extra->originator_name));
	  } else {
		  stream_putc (s, strlen(from->remote_name));
		  stream_put (s, from->remote_name, strlen(from->remote_name));
	  }
      ...
      if (attr->extra && attr->extra->cluster)
	{
	  /* If this peer configuration's parent BGP has cluster_id. */
	  if (bgp->config & BGP_CONFIG_CLUSTER_ID) {
		  stream_putw (s, attr->extra->cluster->length + strlen(bgp->cluster_name) + 1);
		  stream_putc (s, strlen(bgp->cluster_name));
		  stream_put (s, bgp->cluster_name, strlen(bgp->cluster_name));
	  } else {
		  stream_putw (s, attr->extra->cluster->length + strlen(bgp->router_name) + 1);
		  stream_putc (s, strlen(bgp->router_name));
		  stream_put (s, bgp->router_name, strlen(bgp->router_name));
	  }
	  stream_put (s, attr->extra->cluster->list,
	              attr->extra->cluster->length);
	}
      else
	{
	  /* If this peer configuration's parent BGP has cluster_id. */
	  if (bgp->config & BGP_CONFIG_CLUSTER_ID) {
		stream_putw (s, strlen(bgp->cluster_name)+1);
		stream_putc (s, strlen(bgp->cluster_name));
		stream_put (s, bgp->cluster_name, strlen(bgp->cluster_name));
	  } else {
		stream_putw (s, strlen(bgp->router_name)+1);
		stream_putc (s, strlen(bgp->router_name));
		stream_put (s, bgp->router_name, strlen(bgp->router_name));
	  }
	}
