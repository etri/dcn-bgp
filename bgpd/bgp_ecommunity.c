...
ecommunity_parse (u_int8_t *pnt, u_short length)
{
  ...
  /* Create a new Extended Communities Attribute by uniq and sort each
     Extended Communities value  */
  new1 = ecommunity_uniq_sort (&tmp);

  return ecommunity_intern (new1);
}
...
ecommunity_dup (struct ecommunity *ecom)
{
  ...
  new1 = XCALLOC (MTYPE_ECOMMUNITY, sizeof (struct ecommunity));
  new1->size = ecom->size;
  if (new1->size)
    {
      new1->val = XMALLOC (MTYPE_ECOMMUNITY_VAL, ecom->size * ECOMMUNITY_SIZE);
      memcpy (new1->val, ecom->val, ecom->size * ECOMMUNITY_SIZE);
    }
  else
    new1->val = NULL;
  return new1;
