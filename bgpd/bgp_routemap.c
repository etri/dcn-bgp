      ...
      void *object)
{
  ...
  su_def.sin.sin_family = AF_INET;
  su_def.sin.sin_addr.s_addr = INADDR_ANY;
#endif
...
route_set_aspath_prepend (void *rule, struct prefix *prefix, route_map_object_t type, void *object)
{
  ...
  if (type == RMAP_BGP)
	...
	new1 = aspath_dup (binfo->attr->aspath);
      else
	new1 = binfo->attr->aspath;

      if ((uintptr_t)rule > 10)
      {
	aspath = rule;
	aspath_prepend (aspath, new1);
      }
      else
      {
	as_t as = aspath_leftmost(new1);
	if (!as) as = binfo->peer->as;
	new1 = aspath_add_seq_n (new1, as, (uintptr_t) rule);
      }

      binfo->attr->aspath = new1;
    }
		     ...
		     route_map_object_t type, void *object)
{
  ...
  if (type == RMAP_BGP)
      ...
      if (rcs->additive && old)
	{
	  ...
	  if (old->refcnt == 0)
	    community_free (old);
	  new1 = community_uniq_sort (merge);
	  community_free (merge);
	}
      else
	new1 = community_dup (rcs->com);
		     ...
		     route_map_object_t type, void *object)
{
  ...
  if (type == RMAP_BGP)
      ...
      if (rcs->additive && old)
	{
	  ...
	  if (old->refcnt == 0)
	    lcommunity_free (&old);
	  new1 = lcommunity_uniq_sort (merge);
	  lcommunity_free (&merge);
	}
      else
        {
          new1 = lcommunity_dup (rcs->lcom);
        }

      /* will be interned by caller if required */
      bgp_attr_extra_get (attr)->lcommunity = new1;
      attr->flag |= ATTR_FLAG_BIT (BGP_ATTR_LARGE_COMMUNITIES);
    }
	  ...
	  if (new1->size == 0)
	    {
	      ...
	      lcommunity_free (&new1);
	    }
	  else
	    {
	      binfo->attr->extra->lcommunity = new1;
	      binfo->attr->flag |= ATTR_FLAG_BIT (BGP_ATTR_LARGE_COMMUNITIES);
	    }
	}
    }
