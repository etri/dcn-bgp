...
// Added by MODORI on 20210908
#include "bgpd/dcn_master.hpp"
#include "bgpd/dcn_common.hpp"
...
// added by chahn on 211202
static struct peer *
dcn_peer_and_group_lookup_vty (struct vty *vty, const char *name)
{
  int ret;
  struct bgp *bgp;
  union sockunion su;
       ...
       AS_STR)
{
    ...
    // added by MODORI on 20210923
    bgp_option_set(BGP_OPT_NO_LISTEN);
    //get_dcn_master()->setSyncStrategy(as);
    if (argc == 2)
        name = argv[1];
...
// added by MODORI on 20210910
DEFUN (bgp_router_name,
       bgp_router_name_cmd,
       "bgp router-name NAME",
       BGP_STR
       ...
       "Manually configured router identifier\n")
{
  ...
  bgp_router_name_static_set (bgp, argv[0]);
  get_dcn_master()->register_router_name(argv[0]);
...
// added by MODORI on 20210928
DEFUN (no_bgp_router_name,
       no_bgp_router_name_cmd,
       "no bgp router-name",
       NO_STR
       BGP_STR
       "Override configured router identifier\n")
{
    ...
    if (argc == 1)
    {
        ...
        if (! ret)
        {
            vty_out (vty, "%% Malformed BGP router name%s", VTY_NEWLINE);
            return CMD_WARNING;
        }

        if ( strcmp (bgp->router_name_static, argv[0]))
        {
            vty_out (vty, "%% BGP router-name doesn't match%s", VTY_NEWLINE);
            return CMD_WARNING;
        }
    }

    get_dcn_master()->unregister_router_name(bgp->router_name_static);
    bgp_router_name_static_set (bgp, NULL);

    return CMD_SUCCESS;
}
ALIAS (no_bgp_router_name,
       no_bgp_router_name_val_cmd,
       "no bgp router-name WORD",
       NO_STR
       BGP_STR
       "Override configured router name\n"
       "Manually configured router name\n")
...
/* BGP Cluster ID.  */
DEFUN (bgp_cluster_name,
       bgp_cluster_name_cmd,
       "bgp cluster-name WORD",
       BGP_STR
       "Configure Route-Reflector Cluster-Name\n"
       "Route-Reflector Cluster-name in ndn::Name format\n")
{
  int ret;
  struct bgp *bgp;
  bgp = vty->index;
...
#endif
  dcn_bgp_cluster_id_set (bgp, argv[0]);
...
// added by MODORI on 20210923
static int
dcn_peer_remote_as_vty (struct vty *vty, const char *peer_str/*peer router-name*/, 
                    const char *as_str, const char *uri_str, afi_t afi, safi_t safi)
{
    int ret;
    struct bgp *bgp;
    as_t as;
    ...
    if(uri_str!=NULL)
    std::cout << "dcn_peer_remote_as_vty: " << peer_str << ", " << as_str << ", uri: " << uri_str << std::endl;
    else
    std::cout << "dcn_peer_remote_as_vty: " << peer_str << ", " << as_str << std::endl;
...
#if 0
    std::cout << "bgp->as: " << bgp->as << ", peer->as: " << as << std::endl;

    uint64_t face_id= get_dcn_master()->register_peer_router(peer_str, uri_str);
    if(face_id > 0 ){
        if( bgp->as == as ){ // iBGP
            //get_dcn_master()->add_peer_router_to_ibgp(peer_str);
        }else{
        }
    }else{
            vty_out (vty, "%% There is no face%s", VTY_NEWLINE);
            return CMD_WARNING;
    }
#endif
// modified by MODORI on 20211027
    ret = dcn_peer_remote_as (bgp, peer_str, &as, afi, safi, uri_str);
...
// added by MODORI on 20210923
static int
dcn_peer_remote_as_face_vty (struct vty *vty, const char *peer_str/*peer router-name*/,
                    const char *as_str, const char *face_str, afi_t afi, safi_t safi)
{
    int ret;
    struct bgp *bgp;
    as_t as;
    ...
    std::cout << "dcn_peer_remote_as_vty: " << peer_str << ", " << as_str << ", face: " << face_str << std
::endl;
    ...
    uint64_t face_id= get_dcn_master()->register_peer_router(peer_str, face_str);
    if(face_id > 0 ){
        if( bgp->as == as ){ // iBGP
            //get_dcn_master()->add_peer_router_to_ibgp(peer_str);
        }else{
...
// modified by MODORI on 20211027
    ret = dcn_peer_remote_as (bgp, peer_str, &as, afi, safi, face_str);
...
DEFUN (neighbor_dcn_remote_as,
       neighbor_dcn_remote_as_cmd,
       NEIGHBOR_CMD2 "remote-as " CMD_AS_RANGE " uri  WORD",
       "Specify a BGP neighbor\n"
       AS_STR
		"Specify a Remote Uri1"
		"Specify a Remote Uri"
)
{
  return dcn_peer_remote_as_vty (vty, argv[0], argv[1], argv[2], AFI_NDN, SAFI_NDN);
}

DEFUN (neighbor_dcn_remote_as1,
        neighbor_dcn_remote_as_cmd1,
...
{
    return dcn_peer_remote_as_vty (vty, argv[0], argv[1], NULL, AFI_NDN, SAFI_NDN);
}

DEFUN (neighbor_dcn_remote_as_face,
       neighbor_dcn_remote_as_face_cmd,
       NEIGHBOR_CMD2 "remote-as " CMD_AS_RANGE " face " CMD_FACE_RANGE,
       "Specify a BGP neighbor\n"
       AS_STR
                "Specify a Remote Uri1"
                "Specify a Remote Uri"
...
{
  return dcn_peer_remote_as_face_vty (vty, argv[0], argv[1], argv[2], AFI_NDN, SAFI_NDN);
}
       ...
       NEIGHBOR_ADDR_STR2)
{
  ...
  std::cout << "no_neighbor_cmd" << std::endl;
  ...
  struct bgp *bgp = vty->index;
   // char router_name[1024]={0,};
  //  sprintf(router_name, "%s/DBGP", argv[0]);

  peer = dcn_peer_lookup (bgp, argv[0]);
  //peer = dcn_peer_lookup (bgp, router_name);
  if (peer){

	  bgp_notify_send (peer, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_PEER_UNCONFIG);
      get_dcn_master()->unregister_peer_router(argv[0]);
      peer_delete (peer);
...
{
    // modified by modori on 20211207
#if 0
  ...
  peer = dcn_peer_and_group_lookup_vty (vty, peer_str);
  if (! peer)
    return CMD_WARNING;
       ...
       "Configure a neighbor as Route Reflector client\n")
{
  struct peer *peer;
    ...
    //modified by modori 20211207
  //peer = peer_and_group_lookup_vty (vty, argv[0]);
  peer = dcn_peer_and_group_lookup_vty (vty, argv[0]);
  if (! peer)
    return CMD_WARNING;

  return peer_af_flag_set_vty (vty, argv[0], AFI_NDN,
			       SAFI_NDN,
...
DEFUN (no_neighbor_route_reflector_client,
       ...
       "Configure a neighbor as Route Reflector client\n")
{
  return peer_af_flag_unset_vty (vty, argv[0], AFI_NDN,
				 SAFI_NDN,
				 PEER_FLAG_REFLECTOR_CLIENT);
  //return peer_af_flag_unset_vty (vty, argv[0], bgp_node_afi (vty),
//				 bgp_node_safi (vty),
...
// added by chahn on 211202
static int
dcn_peer_weight_set_vty (struct vty *vty, const char *name,
                     const char *weight_str)
{
  struct peer *peer;
  unsigned long weight;

  peer = dcn_peer_and_group_lookup_vty (vty, name);
  if (! peer)
    return CMD_WARNING;
       ...
       "default weight\n")
{
  //char router_name[1024]={0,};
  //sprintf(router_name, "%s/DBGP", argv[0]);
  return dcn_peer_weight_set_vty (vty, argv[0], argv[1]);
}
...
// added by MODORI on 20210910
DEFUN (address_family_ndn,
       address_family_ndn_cmd,
       "address-family ndn",
       "Enter Address Family command mode\n"
       "Address family\n")
{
  vty->node = BGP_NDN_NODE;
  return CMD_SUCCESS;
}

DEFUN (address_family_ndn_safi,
       address_family_ndn_safi_cmd,
       "address-family ndn (unicast|multicast)",
       "Enter Address Family command mode\n"
       "Address family\n"
       ...
       "Exit from Address Family configuration mode\n")
{
  ...
  if (vty->node == BGP_IPV4_NODE
      //added by MODORI on 20100818
      //|| vty->node == NDN_NODE
      //added by MODORI on 20100910
      || vty->node == BGP_NDN_NODE
      || vty->node == BGP_NDNM_NODE
...
static int
dcn_bgp_show_summary (struct vty *vty, struct bgp *bgp, int afi, int safi)
{
    struct peer *peer;
    struct listnode *node, *nnode;
    ...
    static char header[] = "Neighbor                 AS MsgRcvd MsgSent   TblVer  InQ OutQ Up/Down  State/PfxRcd";
    ...
    for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
    {
        if (peer->afc[afi][safi])
        {
            if (!count)
            {
                ...
                vty_out (vty,
                        "BGP router name %s, local AS number %u%s",
                        //added by MODORI on 20210921
                        //inet_ntoa (bgp->router_id), bgp->as, VTY_NEWLINE);
                        bgp->router_name!=NULL?bgp->router_name:"N/A", bgp->as, VTY_NEWLINE);
	    ...
	    // added by MODORI on 20211028
            //len = vty_out (vty, "%s", peer->host);
            len = vty_out (vty, "%s", peer->remote_name);
#if 0 
            len = 16 - len;
            if (len < 1)
    ...
    if (count)
    {
        ...
        vty_out (vty, "No %s neighbor is configured%s",
                afi == AFI_IP ? "IPv4" : "NDN", VTY_NEWLINE);
    return CMD_SUCCESS;
}
                      ...
                      afi_t afi, safi_t safi)
{
  ...
  // added by MODORI on 20211028
	if( afi == AFI_NDN )
      dcn_bgp_show_summary (vty, bgp, afi, safi);
	else
...
/* `show ip bgp summary' commands. */
DEFUN (show_ndn_bgp_summary,
       show_ndn_bgp_summary_cmd,
       "show dcn bgp summary",
       SHOW_STR
       IP_STR
       BGP_STR
       "Summary of BGP neighbor status\n")
{
  return bgp_show_summary_vty (vty, NULL, AFI_NDN, SAFI_NDN);
}
...
DEFUN (show_bgp_summary,
       ...
       "Summary of BGP neighbor status\n")
{
    vty_out(vty, "%sNDN Summary:%s", VTY_NEWLINE, VTY_NEWLINE);
    vty_out(vty, "---------------------%s", VTY_NEWLINE);
    bgp_show_summary_vty (vty, NULL, AFI_NDN, SAFI_NDN);
#if 0
    vty_out(vty, "%sIPv4 Unicast Summary:%s", VTY_NEWLINE, VTY_NEWLINE);
    vty_out(vty, "---------------------%s", VTY_NEWLINE);
...
bgp_show_peer (struct vty *vty, struct peer *p)
{
  ...
  vty_out (vty, "  BGP version NDN");
  vty_out (vty, ", remote router ID %s%s", 
		  // added by MODORI 20211028
...
static int
dcn_bgp_show_neighbor_vty (struct vty *vty, const char *name,
                       enum show_type type, const char *ip_str)
{
  int ret;
  struct bgp *bgp;
...
// added by MODORI on 20211028
/* "show ip bgp neighbors" commands.  */DEFUN (show_ndn_bgp_neighbors,
       show_ndn_bgp_neighbors_cmd,
       "show dcn bgp neighbors",
       SHOW_STR
       "NDN Information"
       BGP_STR
       "Detailed information on NDN and BGP neighbor connections\n")
{
  return dcn_bgp_show_neighbor_vty (vty, NULL, show_all, NULL);
}
...
DEFUN (bgp_redistribute_ndn,
       bgp_redistribute_ndn_cmd,
       ...
       QUAGGA_IP_REDIST_HELP_STR_BGPD)
{
  ...
  type = proto_redistnum (AFI_NDN, argv[0]);
  ...
  dcn_bgp_redistribute_set (vty->index, AFI_NDN, type);

  return get_dcn_master()->redistribute_static();
}
...
DEFUN (no_bgp_redistribute_ndn,
       no_bgp_redistribute_ndn_cmd,
       ...
       QUAGGA_IP_REDIST_HELP_STR_BGPD)
{
  ...
  //dcn_bgp_redistribute_unset (vty->index, AFI_NDN, type);
  return get_dcn_master()->no_redistribute_static();
}

ALIAS (no_bgp_redistribute_ndn,
...
// added by MODORI on 20210910
static struct cmd_node bgp_ndn_multicast_node =
{
  BGP_NDNM_NODE,
  "%s(config-router-af)# ",
  1,
};
static struct cmd_node bgp_ndn_unicast_node =
{
  BGP_NDN_NODE,
  "%s(config-router-af)# ",
  1,
...
bgp_vty_init (void)
{
  ...
  //added by MODORI on 20210818
  install_node (&bgp_ndn_unicast_node, NULL);
  install_node (&bgp_ndn_multicast_node, NULL);
  ...
  //added by MODORI on 20210910
  install_default (BGP_NDN_NODE);
  install_default (BGP_NDNM_NODE);
  ...
  // added by MODORI on 20210910
  install_element (BGP_NODE, &bgp_router_name_cmd);
  install_element (BGP_NODE, &no_bgp_router_name_cmd);
  install_element (BGP_NODE, &no_bgp_router_name_val_cmd);
  ...
  install_element (BGP_NODE, &bgp_cluster_name_cmd);
#if 0
...
// added by MODORI(20210923)
  //install_element (BGP_NODE, &neighbor_e_dcn_remote_as_cmd);
  //install_element (BGP_NODE, &neighbor_i_dcn_remote_as_cmd);
  install_element (BGP_NODE, &neighbor_dcn_remote_as_cmd);
  install_element (BGP_NODE, &neighbor_dcn_remote_as_cmd1);
  install_element (BGP_NODE, &neighbor_dcn_remote_as_face_cmd);
  ...
  // added by MODORI on 20210910
  install_element (BGP_NODE, &address_family_ndn_cmd);
  install_element (BGP_NODE, &address_family_ndn_safi_cmd);
  ...
  // added by MODORI on 20210910
  install_element (BGP_NDN_NODE, &exit_address_family_cmd);
  install_element (BGP_NDNM_NODE, &exit_address_family_cmd);
  ...
  install_element (BGP_NODE, &bgp_redistribute_ndn_cmd);
  install_element (BGP_NODE, &no_bgp_redistribute_ndn_cmd);
#if 0
  ...
  // added by MODORI on 20211028
  install_element (VIEW_NODE, &show_ndn_bgp_summary_cmd);
#if 0
...
#endif
  // added by MODORI on 20211028
  install_element (RESTRICTED_NODE, &show_ndn_bgp_summary_cmd);
#if 0
...
#endif
  // added by MODORI on 20211028
  install_element (VIEW_NODE, &show_ndn_bgp_neighbors_cmd);
