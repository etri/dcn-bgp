...
// added by chahn
static void dcn_register_nexthop(struct bgp_nexthop_cache *bnc);
static void unregister_nexthop (struct bgp_nexthop_cache *bnc);
// added by chahn
static void dcn_unregister_nexthop (struct bgp_nexthop_cache *bnc);
...
bgp_get_nexthop_rn (struct bgp_info *path, struct peer *peer)
{
  ...
  else
  {
	  ...
	  {
		  p.family = AF_NDN;
		  p.prefixlen = strlen(peer->remote_name);
		  strcpy(p.name, peer->remote_name);
	  }
	  else
		  return NULL;
  }
...
// added by chahn
int
dcn_bgp_ensure_nexthop (struct bgp_info *ri, struct peer *peer,
                    int connected)
{
  struct bgp_node *rn;
  struct bgp_nexthop_cache *bnc;
  ...
  if (!CHECK_FLAG(bnc->flags, BGP_NEXTHOP_REGISTERED))
	  dcn_register_nexthop(bnc);
	  //get_dcn_master()->get_nexthop_face(bnc->node->p.name);
...
void
dcn_bgp_nexthop_update (char *nexthop_name, uint64_t *face_id, uint8_t nexthop_num)
{
	zlog_debug("dcn_bgp_nexthop_update %s: nexthop_num: %d", nexthop_name, nexthop_num);
	struct bgp_nexthop_cache *bnc;
	struct nexthop *nexthop;
	struct nexthop *oldnh;
	struct nexthop *nhlist_head = NULL;
	...
	p.family = AF_NDN;
	p.prefixlen = strlen(nexthop_name);
	strcpy(p.name, nexthop_name);
	...
	if (!rn || !rn->info)
	{
		if (BGP_DEBUG(nht, NHT))
		{
			zlog_debug("parse nexthop update(%s): rn not found", p.name);
		}
		if (rn)
			bgp_unlock_node (rn);
		return;
	...
	if (BGP_DEBUG(nht, NHT))
	{
		zlog_debug("parse nexthop update(%s): metric=%d, #nexthop=%d", p.name,
				metric, nexthop_num);
	}
	...
	if (nexthop_num) {
		...
		for (i = 0; i < nexthop_num; i++) {
			nexthop = nexthop_new();
			nexthop->type = ZEBRA_NEXTHOP_NDN_FACE;
			nexthop->face_id = face_id[i];
...
make_prefix (int afi, struct bgp_info *ri, struct prefix *p)
{
  ...
  switch (afi)
	...
	case AFI_NDN:
      p->family = AF_NDN;
      p->prefixlen = strlen(ri->attr->name);
	  strcpy(p->name, ri->attr->name);
	  break;
    default:
      break;
    }
...
// added by chahn
static void
dcn_register_nexthop (struct bgp_nexthop_cache *bnc)
{
  /* Check if we have already registered */
  if (bnc->flags & BGP_NEXTHOP_REGISTERED)
    return;
...
// added by chahn
static void
dcn_unregister_nexthop (struct bgp_nexthop_cache *bnc)
{
  /* Check if we have already registered */
  if (!CHECK_FLAG(bnc->flags, BGP_NEXTHOP_REGISTERED))
    return;
