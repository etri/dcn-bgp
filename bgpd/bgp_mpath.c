...
bgp_mpath_is_configured_sort (struct bgp *bgp, bgp_peer_sort_t sort,
                              int afi, safi_t safi)
{
  struct bgp_maxpaths_cfg *cfg = (struct bgp_maxpaths_cfg *)&bgp->maxpaths[afi][safi];
...
bgp_info_mpath_cmp (void *val1, void *val2)
{
  ...
  bi1 = (struct bgp_info *)val1;
  bi2 = (struct bgp_info *)val2;
...
bgp_info_mpath_new (void)
{
  ...
  new_mpath = (struct bgp_info_mpath *)XCALLOC (MTYPE_BGP_MPATH_INFO, sizeof (struct bgp_info_mpath));
  return new_mpath;
}
                        ...
                        int afi, safi_t safi)
{
  ...
  if (new_best)
    {
      mpath_cfg = (struct bgp_maxpaths_cfg *)&new_best->peer->bgp->maxpaths[afi][safi];
      mpath_count++;
  ...
  while (mp_node || cur_mpath)
    {
      ...
      if (cur_mpath && (!mp_node ||
      ...
      else
        {
           ...
           */
          new_mpath = (struct bgp_info *)listgetdata (mp_node);
          list_delete_node (mp_list, mp_node);
