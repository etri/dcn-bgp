...
//#include "bgpd/certificate-store.hpp"
...
static const char *pid_file = "./bgpd.pid";
...
main (int argc, char **argv)
{
  ...
  std::string configFileName("/usr/local/etc/dbgpd-static.conf");

  dcn::DCN_CP confParam(face, keyChain, configFileName);
  dcn::ConfFileProcessor configProcessor(confParam);
  ...
  auto thread= threadGrp.create_thread(boost::bind(&ndn::Face::processEvents, &face, boost::chrono::milliseconds(0), true));
  cpu_set_t  mask;
  CPU_ZERO(&mask);
  CPU_SET(sched_getcpu()+1, &mask);
  pthread_setaffinity_np(thread->native_handle(), sizeof(mask), &mask);
  std::cout <<"bgpd main's core: " << sched_getcpu() << std::endl;
    set_dcn_master( std::make_shared<dcn::DcnMaster>(face, keyChain, confParam) );
    //dcn::security::CertificateStore certStore(face, confParam, *get_dcn_master());
#endif
...
// for Test
    //get_dcn_master()->publish_update_for_ibgp();
