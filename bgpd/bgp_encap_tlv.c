    ...
    struct bgp_tea_subtlv_encap_l2tpv3_over_ip	*st)
{
    ...
    new1 = XCALLOC(MTYPE_ENCAP_TLV, sizeof(struct bgp_attr_encap_subtlv) - 1 + total);
    assert(new1);
    new1->type = BGP_ENCAP_SUBTLV_TYPE_ENCAPSULATION;
    new1->length = total;
    p = new1->value;
    ...
    struct bgp_tea_subtlv_proto_type	*st)
{
    ...
    new1 = XCALLOC(MTYPE_ENCAP_TLV, sizeof(struct bgp_attr_encap_subtlv) - 1 + total);
    assert(new1);
    new1->type = BGP_ENCAP_SUBTLV_TYPE_PROTO_TYPE;
    new1->length = total;
    p = new1->value;
    ...
    struct bgp_tea_subtlv_color	*st)
{
    ...
    new1 = XCALLOC(MTYPE_ENCAP_TLV, sizeof(struct bgp_attr_encap_subtlv) - 1 + total);
    assert(new1);
    new1->type = BGP_ENCAP_SUBTLV_TYPE_COLOR;
    new1->length = total;
    p = new1->value;
    ...
    struct bgp_tea_subtlv_ipsec_ta	*st)
{
    ...
    new1 = XCALLOC(MTYPE_ENCAP_TLV, sizeof(struct bgp_attr_encap_subtlv) - 1 + total);
    assert(new1);
    new1->type = BGP_ENCAP_SUBTLV_TYPE_IPSEC_TA;
    new1->length = total;
    p = new1->value;
    ...
    struct bgp_tea_subtlv_remote_endpoint	*st)
{
    ...
    new1 = XCALLOC(MTYPE_ENCAP_TLV, sizeof(struct bgp_attr_encap_subtlv) - 1 + total);
    assert(new1);
    new1->type = BGP_ENCAP_SUBTLV_TYPE_REMOTE_ENDPOINT;
    new1->length = total;
    p = new1->value;
...
#define ENC_SUBTLV(flag, function, field) do {\
	...
	new1 = function(&bet->field);\
	if (last) {\
	    last->next = new1;\
	} else {\
	    extra->encap_subtlvs = new1;\
	}\
	last = new1;\
    }\
} while (0)
