		       ...
		       const char *name, int master)
{
  ...
  new1 = community_list_new ();
  new1->name = XSTRDUP (MTYPE_COMMUNITY_LIST_NAME, name);
  ...
  if (i == strlen (name))
    {
      new1->sort = COMMUNITY_LIST_NUMBER;
    ...
    {
      new1->sort = COMMUNITY_LIST_STRING;
  ...
  new1->next = point;
  new1->prev = point->prev;

  if (point->prev)
    point->prev->next = new1;
  point->prev = new1;
...
community_list_dup_check (struct community_list *list,
                          struct community_entry *new1)
{
  struct community_entry *entry;

  for (entry = list->head; entry; entry = entry->next)
    {
      if (entry->style != new1->style)
        continue;

      if (entry->direct != new1->direct)
        continue;

      if (entry->any != new1->any)
        continue;
      ...
      switch (entry->style)
        {
        case COMMUNITY_LIST_STANDARD:
          if (community_cmp (entry->u.com, new1->u.com))
            return 1;
          break;
        case EXTCOMMUNITY_LIST_STANDARD:
          if (ecommunity_cmp (entry->u.ecom, new1->u.ecom))
            return 1;
          break;
        case LARGE_COMMUNITY_LIST_STANDARD:
          if (lcommunity_cmp (entry->u.lcom, new1->u.lcom))
            return 1;
          break;
        case COMMUNITY_LIST_EXPANDED:
        case EXTCOMMUNITY_LIST_EXPANDED:
        case LARGE_COMMUNITY_LIST_EXPANDED:
          if (entry->config && new1->config
              && strcmp (entry->config, new1->config) == 0)
            return 1;
          if (!entry->config && !new1->config)
            return 1;
          break;
