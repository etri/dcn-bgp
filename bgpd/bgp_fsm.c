...
bgp_timer_set (struct peer *peer)
{
  ...
  switch (peer->status)
    ...
    case Connect:
      ...
      FD_SET(peer->fd, &bm->master->readfd);
      BGP_TIMER_OFF (peer->t_holdtime);
      BGP_TIMER_OFF (peer->t_keepalive);
      BGP_TIMER_OFF (peer->t_routeadv);
      break;
...
bgp_start_timer (struct thread *thread)
{
  ...
  if (BGP_DEBUG (fsm, FSM))
    zlog (peer->log, LOG_DEBUG,
	  "%s [FSM] Timer (start timer expire).", peer->remote_name);
...
bgp_graceful_restart_timer_expire (struct thread *thread)
{
  ...
  if (BGP_DEBUG (events, EVENTS))
    {
      zlog_debug ("%s graceful restart timer expired", peer->remote_name);
      zlog_debug ("%s graceful restart stalepath timer stopped", peer->remote_name);
    }
...
bgp_graceful_stale_timer_expire (struct thread *thread)
{
  ...
  if (BGP_DEBUG (events, EVENTS))
    zlog_debug ("%s graceful restart stalepath timer expired", peer->remote_name);
...
bgp_stop (struct peer *peer)
{
  ...
  if (peer->status == Established)
	...
	zlog_info ("%%ADJCHANGE: neighbor %s Down %s", peer->remote_name,
	  ...
	  if (BGP_DEBUG (events, EVENTS))
	    {
	      zlog_debug ("%s graceful restart timer started for %d sec",
			  peer->remote_name, peer->v_gr_restart);
	      zlog_debug ("%s graceful restart stalepath timer started for %d sec",
			  peer->remote_name, peer->bgp->stalepath_time);
	    }
  ...
  /* clear peer fd */
  if (peer->fd > 0) {
	  if (FD_ISSET(peer->fd, &bm->master->readfd))
		  FD_CLR(peer->fd, &bm->master->readfd) ;
  }
  ...
  for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
	...
	peer->orf_plist[afi][safi] = NULL;
        ...
        sprintf (orf_name, "%s.%d.%d", peer->remote_name, afi, safi);
...
bgp_stop_with_notify (struct peer *peer, u_char code, u_char sub_code)
{
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
    {
      zlog_info ("%s [Event] Accepting BGP peer is deleted", peer->remote_name);
      peer_delete (peer);
      return -1;
    }
...
bgp_connect_success (struct peer *peer)
{
  ...
  dcn_bgp_nexthop_set(&peer->nexthop, peer);
#if 0
  if (peer->fd < 0)
    {
  ...
  if (BGP_DEBUG (normal, NORMAL))
    {
      ...
      if (! CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
	zlog_debug ("%s open active, local address %s", peer->remote_name,
		    sockunion2str (peer->su_local, buf1, SU_ADDRSTRLEN));
      else
	zlog_debug ("%s passive open", peer->remote_name);
    }
#endif
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
    {
      realpeer = dcn_peer_lookup (peer->bgp, peer->remote_name);
...
bgp_start (struct peer *peer)
{
  ...
  if (BGP_PEER_START_SUPPRESSED (peer))
    {
      if (BGP_DEBUG (fsm, FSM))
        plog_err (peer->log, "%s [FSM] Trying to start suppressed peer"
                  " - this is never supposed to happen!", peer->remote_name);
      return -1;
    }
  ...
  if (CHECK_FLAG (peer->flags, PEER_FLAG_PASSIVE))
    {
      BGP_EVENT_ADD (peer, NDN_connection_open_failed);
      return 0;
    }
  ...
  switch (status) 
  {
	  case CMD_FACE_DONT_EXIST:
		  BGP_EVENT_ADD(peer, NDN_connection_closed);
		  break;
          default:
          break;
  }

  bgp_connect_success (peer);
  return 0;
}
...
#endif
static int
bgp_keepalive_canonical_order(char *lname, char *rname)
{
	int result = memcmp(lname, rname, min(strlen(lname), strlen(rname)));
	if (result == 0)
		result = strlen(lname) - strlen(rname);
...
{
	if (bgp_keepalive_canonical_order(peer->local_name, peer->remote_name) < 0)
		bgp_keepalive_send (peer);
	return 0;
}
...
bgp_fsm_event_error (struct peer *peer)
{
  plog_err (peer->log, "%s [FSM] unexpected packet received in state %s",
	    peer->remote_name, LOOKUP (bgp_status_msg, peer->status));
...
{
  if (BGP_DEBUG (fsm, FSM))
    plog_debug (peer->log, "%s [FSM] Hold timer expire", peer->remote_name);
...
bgp_establish (struct peer *peer)
{
  ...
  if (bgp_flag_check (peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
    zlog_info ("%%ADJCHANGE: neighbor %s Up", peer->remote_name);
  ...
  if (peer->t_gr_restart)
    {
      BGP_TIMER_OFF (peer->t_gr_restart);
      if (BGP_DEBUG (events, EVENTS))
	zlog_debug ("%s graceful restart timer stopped", peer->remote_name);
    }
...
#if 1
      if (CHECK_FLAG (peer->af_cap[AFI_NDN][SAFI_NDN], PEER_CAP_ORF_PREFIX_RM_ADV))
	if (CHECK_FLAG (peer->af_cap[AFI_NDN][SAFI_NDN], PEER_CAP_ORF_PREFIX_SM_RCV)
	    || CHECK_FLAG (peer->af_cap[AFI_NDN][SAFI_NDN], PEER_CAP_ORF_PREFIX_SM_OLD_RCV))
	  SET_FLAG (peer->af_sflags[AFI_NDN][SAFI_NDN], PEER_STATUS_ORF_WAIT_REFRESH);
  bgp_announce_route_all (peer);
...
{
  if (BGP_DEBUG (fsm, FSM))
    zlog (peer->log, LOG_DEBUG, "%s [FSM] bgp_ignore called", peer->remote_name);
  return 0;
}
...
{
  {
    /* Idle state: In Idle state, all events other than BGP_Start is
       ...
       bgp_start(). */
    {bgp_start,  OpenSent},	/* BGP_Start                    */
    {bgp_stop,   Idle},		/* BGP_Stop                     */
    {bgp_stop,   Idle},		/* NDN_connection_open          */
    {bgp_stop,   Idle},		/* NDN_connection_closed        */
    {bgp_ignore, Idle},		/* NDN_connection_open_failed   */
    {bgp_stop,   Idle},		/* NDN_fatal_error              */
  ...
  },
  {
    ...
    {bgp_stop,    Idle},	/* BGP_Stop                     */
    {bgp_connect_success, OpenSent}, /* NDN_connection_open          */
    {bgp_stop, Idle},		/* NDN_connection_closed        */
    {bgp_connect_fail, Active}, /* NDN_connection_open_failed   */
    {bgp_connect_fail, Idle},	/* NDN_fatal_error              */
  ...
  },
  {
    ...
    {bgp_connect_success, OpenSent}, /* NDN_connection_open          */
    {bgp_stop,    Idle},	/* NDN_connection_closed        */
    {bgp_ignore,  Active},	/* NDN_connection_open_failed   */
    {bgp_ignore,  Idle},	/* NDN_fatal_error              */
  ...
  },
  {
    ...
    {bgp_stop,    Idle},	/* BGP_Stop                     */
    {bgp_stop,    Active},	/* NDN_connection_open          */
    {bgp_stop,    Active},	/* NDN_connection_closed        */
    {bgp_stop,    Active},	/* NDN_connection_open_failed   */
    {bgp_stop,    Active},	/* NDN_fatal_error              */
    {bgp_ignore,  Idle},	/* ConnectRetry_timer_expired   */
  ...
  },
  {
    ...
    {bgp_stop,    Idle},	/* NDN_connection_open          */
    {bgp_stop,    Idle},	/* NDN_connection_closed        */
    {bgp_stop,    Idle},	/* NDN_connection_open_failed   */
    {bgp_stop,    Idle},	/* NDN_fatal_error              */
    {bgp_ignore,  Idle},	/* ConnectRetry_timer_expired   */
    {bgp_fsm_holdtime_expire, Idle},	/* Hold_Timer_expired           */
  ...
  },
  {
    ...
    {bgp_stop,                    Clearing}, /* NDN_connection_open          */
    {bgp_stop,                    Clearing}, /* NDN_connection_closed        */
    {bgp_stop,                 Clearing},	/* NDN_connection_open_failed   */
    {bgp_stop,                    Clearing}, /* NDN_fatal_error              */
  ...
  },
  {
    ...
    {bgp_ignore,  Deleted},	/* NDN_connection_open          */
    {bgp_ignore,  Deleted},	/* NDN_connection_closed        */
    {bgp_ignore,  Deleted},	/* NDN_connection_open_failed   */
    {bgp_ignore,  Deleted},	/* NDN_fatal_error              */
...
static const char *bgp_event_str[] =
{
  ...
  "BGP_Stop",
  "NDN_connection_open",
  "NDN_connection_closed",
  "NDN_connection_open_failed",
  "NDN_fatal_error",
...
bgp_event (struct thread *thread)
{
  ...
  if (BGP_DEBUG (fsm, FSM) && peer->status != next)
    plog_debug (peer->log, "%s [FSM] %s (%s->%s)", peer->remote_name,
	       bgp_event_str[event],
	       LOOKUP (bgp_status_msg, peer->status),
...
/* dcn peer connect from data packet */
int dcn_peer_connect_open_success(struct peer *peer)
{
	if (peer->status == OpenSent)
		return CMD_SUCCESS;

	BGP_EVENT_ADD (peer, NDN_connection_open);
	return CMD_SUCCESS;
}

int dcn_peer_connect_open_fail(struct peer *peer)
{
	if (!peer)
		return CMD_ERR_NO_MATCH;

	zlog_debug ("dcn_peer_connect_open_fail: %s", peer?peer->remote_name : "Unknown...");

	BGP_EVENT_ADD (peer, BGP_Start);
	return CMD_SUCCESS;
}

/* dcn peer disconnect from data packet */
int dcn_peer_connect_close_success(struct peer *peer)
{
	zlog_debug ("dcn_peer_connect_close_success");
}

/* dcn peer connect from interest packet */
int dcn_peer_connect_open(struct peer *peer)
{
	if (!peer || peer->status == Idle || peer->status > Established)
	{
		return CMD_ERR_NO_MATCH;
	}
...
/* dcn peer disconnect from interest packet */
int dcn_peer_connect_close(const char *router_name)
{
	zlog_debug ("dcn_peer_connect_close : %s ", router_name);

#if 0
	struct peer *peer;

    peer = dcn_peer_lookup(NULL, router_name);
...
int dcn_peer_open_interest_receive(struct peer *peer, size_t length, const uint8_t * message)
{
	if (peer->status != OpenSent)
		return CMD_ERR_NO_MATCH;

	stream_write (peer->ibuf, message, length);

	bgp_size_t size;
    u_char bgp_type = 0;
    stream_forward_getp (peer->ibuf, BGP_MARKER_SIZE);
    size = stream_getw (peer->ibuf);
    bgp_type = stream_getc (peer->ibuf);
    size = size - BGP_HEADER_SIZE;

	if (bgp_type == BGP_MSG_OPEN) {
		peer->open_in++;
...
int dcn_peer_open_data_receive(struct peer *peer, size_t length, const uint8_t * message)
{
	if (peer->status != OpenSent)
		return CMD_ERR_NO_MATCH;
