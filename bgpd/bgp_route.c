...
{
  struct bgp_info_extra *new1;
  new1 = XCALLOC (MTYPE_BGP_ROUTE_EXTRA, sizeof (struct bgp_info_extra));
  return new1;
}
...
static int
bgp_info_cmp (struct bgp *bgp, struct bgp_info *new1, struct bgp_info *exist,
              afi_t afi, safi_t safi)
{
   ...
   */
  if (! (new1->sub_type == BGP_ROUTE_NORMAL))
     return -1;
  ...
  if (bgp_flag_check (bgp, BGP_FLAG_ALWAYS_COMPARE_MED)
	 ...
	 && confed_as_route)
      ...
      || internal_as_route)
    {
      new_med = bgp_med_value (new1->attr, bgp);
  ...
  if (bgp_mpath_is_configured (bgp, afi, safi))
    {
      ...
      else if (new1->peer->sort == BGP_PEER_IBGP)
        {
          if (aspath_cmp (new1->attr->aspath, exist->attr->aspath))
            return 0;
        }
      else if (new1->peer->as == exist->peer->as)
        return 0;
    }
  ...
  if (! bgp_flag_check (bgp, BGP_FLAG_COMPARE_ROUTER_ID)
    ...
    {
      if (CHECK_FLAG (new1->flags, BGP_INFO_SELECTED))
	return -1;
  ...
  if (newattr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID))
    ...
    new_id.s_addr = new1->peer->remote_id.s_addr;
  ...
  if (CHECK_FLAG (exist->flags, BGP_INFO_STALE))
    return -1;
  if (CHECK_FLAG (new1->flags, BGP_INFO_STALE))
    return 1;
  /* locally configured routes to advertise do not have su_remote */
  if (new1->peer->su_remote == NULL)
    return 1;
  if (exist->peer->su_remote == NULL)
    return -1;
  
  ret = sockunion_cmp (new1->peer->su_remote, exist->peer->su_remote);
...
{
	char cluster_name[PREFIX_NAME_LEN];

	if (attr->extra && attr->extra->cluster)
	{
		if (peer->bgp->config & BGP_CONFIG_CLUSTER_ID) {
			strcpy(cluster_name, peer->bgp->cluster_name);
		} else {
			strcpy(cluster_name, peer->bgp->router_name);
		}

		if (cluster_loop_check (attr->extra->cluster, cluster_name))
			return 1;
	}
	return 0;
}
		    ...
		    struct attr *attr, afi_t afi, safi_t safi)
{
  ...
  if (peer && from && peer->sort == BGP_PEER_IBGP &&
      ...
      (! (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID))))
    {
	  ...
	  strcpy(attr->extra->originator_name, from->remote_name);
...
#define NEXTHOP_IS_NDN (\
    (safi == SAFI_NDN && p->family == AF_NDN))
  ...
  if (transparent
      ...
      if (NEXTHOP_IS_V6)
	{
      ...
      if (NEXTHOP_IS_NDN)
	{
	    strcpy (attr->name, peer->nexthop.name);
	}
    }
...
bgp_process_main (struct work_queue *wq, void *data)
{
      ...
      if (! CHECK_FLAG (old_select->flags, BGP_INFO_ATTR_CHANGED))
        {
	      ...
	      CHECK_FLAG (old_select->flags, BGP_INFO_MULTIPATH_CHG))
            get_dcn_master()->add_network(p->name, old_select->peer->face_id, 0);
  ...
  /* FIB update. */
  if ((safi == SAFI_UNICAST || safi == SAFI_MULTICAST || safi == SAFI_NDN) && (! bgp->name &&
      ! bgp_option_check (BGP_OPT_NO_FIB)))
    {
      if (new_select 
	  && new_select->type == ZEBRA_ROUTE_BGP 
	  && new_select->sub_type == BGP_ROUTE_NORMAL) {
		  zlog_debug ("call NDN RIB prefix = %s, nexthop = %s", p->name, new_select->peer->remote_name);
		  if (old_select) {
			  get_dcn_master()->update_nexthop(p->name, new_select->peer->face_id, 0);
		  } else {
			  get_dcn_master()->add_network(p->name, new_select->peer->face_id, 0);
		  }
      ...
      else
	{
	  ...
	  {
	    get_dcn_master()->remove_network(p->name, old_select->peer->face_id);
	  }
	}
    }
	   ...
	   struct bgp_node *rn)
{
  ...
  /* Make new BGP info. */
  new1 = XCALLOC (MTYPE_BGP_ROUTE, sizeof (struct bgp_info));
  new1->type = type;
  new1->sub_type = sub_type;
  new1->peer = peer;
  new1->attr = attr;
  new1->uptime = bgp_clock ();
  new1->net = rn;
  return new1;
}
      ...
      int sub_type, struct prefix_rd *prd, u_char *tag)
{
  ...
  new1 = info_make(type, sub_type, peer, attr_new, rn);

  /* Update MPLS tag. */
  if (safi == SAFI_MPLS_VPN)
    memcpy ((bgp_info_extra_get (new1))->tag, tag, 3);

  bgp_info_set_flag (rn, new1, BGP_INFO_VALID);

  /* Register new BGP information. */
  bgp_info_add (rn, new1);
	    ...
	    struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  ...
  if ((afi == AFI_IP || afi == AFI_IP6) && safi == SAFI_UNICAST)
	...
	connected = 0;
      ...
      if (bgp_ensure_nexthop (new1, NULL, connected))
	bgp_info_set_flag (rn, new1, BGP_INFO_VALID);
      else
	{
	  if (BGP_DEBUG(nht, NHT))
	  ...
	  bgp_info_unset_flag (rn, new1, BGP_INFO_VALID);
	}
    }
  else
    bgp_info_set_flag (rn, new1, BGP_INFO_VALID);
  ...
  /* Increment prefix */
  bgp_aggregate_increment (bgp, p, new1, afi, safi);
...
static int
dcn_bgp_update_main (struct peer *peer, struct prefix *p, struct attr *attr,
	    afi_t afi, safi_t safi, int type, int sub_type,
	    struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  int ret;
  ...
  /* Route reflector originator ID check.  */
  if (attr->flag & ATTR_FLAG_BIT (BGP_ATTR_ORIGINATOR_ID)
      && strcmp(bgp->router_name, attr->extra->originator_name) == 0)
    {
      reason = "originator is us;";
      goto filtered;
    }
  ...
  if (ri)
      ...
      if (afi == AFI_NDN && safi == SAFI_NDN)
	{
	  ...
	  if (dcn_bgp_ensure_nexthop (ri, NULL, connected))
	    bgp_info_set_flag (rn, ri, BGP_INFO_VALID);
	  else
	    {
	      if (BGP_DEBUG(nht, NHT))
  ...
  if (afi == AFI_NDN && safi == SAFI_NDN)
	...
	connected = 0;
      ...
      if (dcn_bgp_ensure_nexthop (new1, NULL, connected))
	bgp_info_set_flag (rn, new1, BGP_INFO_VALID);
      else
	{
	  if (BGP_DEBUG(nht, NHT))
...
int
dcn_bgp_update (struct peer *peer, struct prefix *p, struct attr *attr,
            afi_t afi, safi_t safi, int type, int sub_type,
            struct prefix_rd *prd, u_char *tag, int soft_reconfig)
{
  struct peer *rsclient;
  ...
  ret = dcn_bgp_update_main (peer, p, attr, afi, safi, type, sub_type, prd, tag,
          soft_reconfig);
                   ...
                   struct bgp_table *table, int rsclient)
{
  ...
  struct attr_extra extra;
    ...
    zlog (peer->log, LOG_DEBUG, "&***bgp_announce_table");
...
#else
      bgp_announce_route (peer, AFI_NDN, SAFI_NDN);
#endif
}
    ...
    for (ri = rn->info; ri; ri = next)
      {
		...
		{
		  get_dcn_master()->remove_network(rn->p.name, ri->peer->face_id);
		}
      }
}
...
int
bgp_nlri_parse_ndn (struct peer *peer, struct attr *attr,
  ...
  for (; pnt < lim; pnt += psize)
    {
      ...
      memcpy (p.name, pnt, psize);

      /* Normal process. */
      if (attr)
	ret = dcn_bgp_update (peer, &p, attr, packet->afi, packet->safi, 
                            ...
                            afi_t afi, safi_t safi)
{
  ...
  /* Make new BGP info. */
  new1 = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_STATIC, bgp->peer_self,
		  attr_new, rn);
  /* Nexthop reachability check. */
  if (bgp_flag_check (bgp, BGP_FLAG_IMPORT_CHECK))
    {
      if (bgp_ensure_nexthop (new1, NULL, 0))
	bgp_info_set_flag (rn, new1, BGP_INFO_VALID);
      else
	{
	  if (BGP_DEBUG(nht, NHT))
			...
			struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
  ...
  /* Make new BGP info. */
  new1 = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_STATIC, bgp->peer_self, attr_new,
		  rn);
  /* Nexthop reachability check. */
  if (bgp_flag_check (bgp, BGP_FLAG_IMPORT_CHECK))
    {
                        ...
                        struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
  ...
  new1 = info_make (ZEBRA_ROUTE_BGP, BGP_ROUTE_STATIC, bgp->peer_self,
		   attr_new, rn);
  SET_FLAG (new1->flags, BGP_INFO_VALID);
  new1->extra = bgp_info_extra_new();
  memcpy (new1->extra->tag, bgp_static->tag, 3);
...
/* Configure static BGP network.  When user don't run zebra, static
   route should be installed as valid.  */
static int
dcn_bgp_static_unset (struct vty *vty, struct bgp *bgp, const char *name,
                afi_t afi, safi_t safi, const char *rmap, int backdoor)
{
  int ret;
  struct prefix p;
...
// added by MODORI on 20211111
/* Configure static BGP network.  When user don't run zebra, static
   route should be installed as valid.  */
static int
dcn_bgp_static_set (struct vty *vty, struct bgp *bgp, const char *name,
                afi_t afi, safi_t safi, const char *rmap, int backdoor)
{
  int ret;
  struct prefix p;
...
// added by MODORI on 20211111
DEFUN (dcn_bgp_network,
       dcn_bgp_network_cmd,
       "network WORD",
       "Specify a network to announce via BGP\n"
       "name prefix\n")
{
  return dcn_bgp_static_set (vty, vty->index, argv[0],
			 AFI_NDN, SAFI_NDN, NULL, 0);
}
DEFUN (no_dcn_bgp_network,
       no_dcn_bgp_network_cmd,
       "no network WORD",
       "Specify a network to announce via BGP\n"
       "name prefix\n")
{
  return dcn_bgp_static_unset (vty, vty->index, argv[0],
			 AFI_NDN, SAFI_NDN, NULL, 0);
}

DEFUN (dcn_bgp_network_bulk,
          dcn_bgp_network_bulk_cmd,
          "bulk-test WORD",
          "BulkTest a network to announce via BGP\n"
          )
{
    char line[1024]={0,};
    ...
    while ( !feof(fp) ) {
        ...
        zlog_debug ("%d: %s", line_cnt, line);

        dcn_bgp_static_set (vty, vty->index, line,
                AFI_NDN, SAFI_NDN, NULL, 0);
    }
    fclose(fp);
    return CMD_SUCCESS;
...
DEFUN (no_dcn_bgp_network_bulk,
        no_dcn_bgp_network_bulk_cmd,
          "no bulk-test",
          "BulkTest a network to announce via BGP\n"
        )
{
    ...
    for(int i=0;i<count;i++){
        dcn_bgp_static_unset (vty, vty->index, argv[0],
            AFI_NDN, SAFI_NDN, NULL, 0);
    }
  return CMD_SUCCESS;
}
		     ...
		     struct bgp_aggregate *aggregate)
{
  ...
  if (aggregate->count > 0)
    {
      rn = bgp_node_get (table, p);
      new1 = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_AGGREGATE, bgp->peer_self,
		      ...
		      u_int32_t metric, u_char type, route_tag_t tag)
{
  ...
  for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
      ...
      if (bgp->redist[afi][type])
	{
	  ...
	  new1 = info_make(type, BGP_ROUTE_REDISTRIBUTE, bgp->peer_self,
			  new_attr, bn);
	  SET_FLAG (new1->flags, BGP_INFO_VALID);

	  bgp_aggregate_increment (bgp, p, new1, afi, SAFI_UNICAST);
	  bgp_info_add (bn, new1);
	  bgp_unlock_node (bn);
	  bgp_process (bgp, bn, afi, SAFI_UNICAST);
	}
    }
...
// added by chahn
void
dcn_bgp_redistribute_add (struct prefix *p,
		      u_int32_t metric, u_char type, route_tag_t tag)
{
	struct bgp *bgp;
	struct listnode *node, *nnode;
		...
		if (bgp->redist[afi][type])
		{
			...
			if (bgp->rmap[afi][type].name)
			{
				...
				if (ret == RMAP_DENYMATCH)
				{
					...
					aspath_unintern (&attr.aspath);
					bgp_attr_extra_free (&attr);
					dcn_bgp_redistribute_delete (p, type);
					return;
				}
			}

			bn = bgp_afi_node_get (bgp->rib[afi][SAFI_NDN],
					afi, SAFI_NDN, p, NULL);
			...
			if (bi)
			{
				...
				else
				{
					...
					if (CHECK_FLAG(bi->flags, BGP_INFO_REMOVED))
						...
						bgp_aggregate_decrement (bgp, p, bi, afi, SAFI_NDN);
					bgp_attr_unintern (&bi->attr);
					bi->attr = new_attr;
					bi->uptime = bgp_clock ();

					/* Process change. */
					bgp_aggregate_increment (bgp, p, bi, afi, SAFI_NDN);
					bgp_process (bgp, bn, afi, SAFI_NDN);
					bgp_unlock_node (bn);
					aspath_unintern (&attr.aspath);
					bgp_attr_extra_free (&attr);
					return;
			...
			bgp_aggregate_increment (bgp, p, new1, afi, SAFI_NDN);
			bgp_info_add (bn, new1);
			bgp_unlock_node (bn);
			bgp_process (bgp, bn, afi, SAFI_NDN);
		}
...
// added by chahn
void
dcn_bgp_redistribute_delete (struct prefix *p, u_char type)
{
	struct bgp *bgp;
	struct listnode *node, *nnode;
	afi_t afi;
	...
	for (ALL_LIST_ELEMENTS (bm->bgp, node, nnode, bgp))
	{
		...
		if (bgp->redist[afi][type])
		{
			rn = bgp_afi_node_get (bgp->rib[afi][SAFI_NDN], afi, SAFI_NDN, p, NULL);
			...
			if (ri)
			{
				bgp_aggregate_decrement (bgp, p, ri, afi, SAFI_NDN);
				bgp_info_delete (rn, ri);
				bgp_process (bgp, rn, afi, SAFI_NDN);
			}
			bgp_unlock_node (rn);
		}
	}
...
// added by chahn
void
dcn_bgp_redistribute_withdraw (struct bgp *bgp, afi_t afi, int type)
{
  struct bgp_node *rn;
  struct bgp_info *ri;
  struct bgp_table *table;

  table = bgp->rib[afi][SAFI_NDN];
  ...
  for (rn = bgp_table_top (table); rn; rn = bgp_route_next (rn))
	...
	if (ri->peer == bgp->peer_self
      ...
      if (ri)
	{
	  bgp_aggregate_decrement (bgp, &rn->p, ri, afi, SAFI_NDN);
	  bgp_info_delete (rn, ri);
	  bgp_process (bgp, rn, afi, SAFI_NDN);
	}
    }
...
// added by chahn on 20211201
static void
dcn_route_vty_out_route (struct prefix *p, struct vty *vty)
{
  int len;
  u_int32_t destination; 
  char buf[BUFSIZ];

  len = vty_out (vty, "%s", p->name);
	...
	if (attr) 
	{
		...
		if (p->family == AF_NDN)
		{
			...
			if (strlen(attr->name) == 0)
				len = vty_out (vty, "%s", "null");
			else
				len = vty_out (vty, "%s", attr->name);
			len = 20 - len;
			if (len < 1) 
				vty_out (vty, "%s%*s", VTY_NEWLINE, 40, " ");
			else
				vty_out (vty, "%*s", len, " ");
		}
	  ...
	  if (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST))
	    {
		  ...
		  while (i < attr->extra->cluster->length) {
              uint8_t name_length = attr->extra->cluster->list[i];
              i++;
              char name[PREFIX_NAME_LEN];
              memcpy(name, &attr->extra->cluster->list[i], name_length);
              i = i + name_length;
              vty_out (vty, "%s ", name);
          }
	    }
...
// added by chahn on 20211201
static int
dcn_bgp_show_table (struct vty *vty, struct bgp_table *table, char *router_name,
	  enum bgp_show_type type, void *output_arg)
{
  struct bgp_info *ri;
  struct bgp_node *rn;
	...
	for (ri = rn->info; ri; ri = ri->next)
	  {
	    ...
	    if (header)
	      {
		vty_out (vty, "BGP table version is 0, local router ID is %s%s", router_name, VTY_NEWLINE);
		vty_out (vty, BGP_SHOW_SCODE_HEADER, VTY_NEWLINE, VTY_NEWLINE);
		vty_out (vty, BGP_SHOW_OCODE_HEADER, VTY_NEWLINE, VTY_NEWLINE);
		if (type == bgp_show_type_dampend_paths
		    || type == bgp_show_type_damp_neighbor)
	    ...
	    if (type == bgp_show_type_dampend_paths
		|| type == bgp_show_type_damp_neighbor)
	      damp_route_vty_out (vty, &rn->p, ri, display, SAFI_NDN);
	    else if (type == bgp_show_type_flap_statistics
		     || type == bgp_show_type_flap_address
		     || type == bgp_show_type_flap_prefix
		     || type == bgp_show_type_flap_cidr_only
		     ...
		     || type == bgp_show_type_flap_route_map
		     || type == bgp_show_type_flap_neighbor)
	      flap_route_vty_out (vty, &rn->p, ri, display, SAFI_NDN);
	    else
	      dcn_route_vty_out (vty, &rn->p, ri, display, SAFI_NDN);
	    display++;
	  }
	if (display)
	  output_count++;
         ...
         enum bgp_show_type type, void *output_arg)
{
  ...
  if (afi == AFI_NDN)
	  return dcn_bgp_show_table (vty, table, bgp->router_name, type, output_arg);
...
DEFUN (show_ndn_bgp,
       ...
       BGP_STR)
{
  return bgp_show (vty, NULL, AFI_NDN, SAFI_NDN, bgp_show_type_normal, NULL);
}
...
{
  struct peer_pcounts pcounts = { peer };
  unsigned int i;
...
bgp_route_init (void)
{
  ...
  /* DCN BGP commands. */
  install_element (BGP_NODE, &dcn_bgp_network_cmd);
  install_element (BGP_NODE, &no_dcn_bgp_network_cmd);
  install_element (BGP_NODE, &dcn_bgp_network_bulk_cmd);
  install_element (BGP_NODE, &no_dcn_bgp_network_bulk_cmd);

  install_element (ENABLE_NODE, &show_ndn_bgp_cmd);
#if 0
  /* IPv4 BGP commands. */
  ...
  // added by chahn on 20211201
  install_element (VIEW_NODE, &show_ndn_bgp_cmd);
